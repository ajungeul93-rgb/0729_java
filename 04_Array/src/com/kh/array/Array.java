...
public class Array {
	...
	// 변수(Variable)
	// 변수란?
	// 메모리(RAM) 공간에 DATE(VALUE)값을 저장하는 공간
	...
	public void method1() {
		 ...
		 * 배열 선언 방법
		 * 
		 * 1) 자료형 배열식별자[]; 2) 자료형[] 배열식별자; --> 요 방법 사용
		 */
		...
		// 현시점에서 우리가 배열을 사용해서 얻을 수 있는 장점 : 반복문 사용가능
	...
	public void methodA() {
		 ...
		 * double[2]; // 0, 1
		 * 
		 * // 배열식별자.length : 배열의 크기(길이) => 정수 System.out.println(integers.length);
	...
	public void methodC() {
		...
		for (int i = 0; i < nums.length; i++) {
			...
			nums[i] = sc.nextInt();
			System.out.println("nums라는 배열의 " + i + "번 째 인덱스 값: " + nums[i]);
		}
